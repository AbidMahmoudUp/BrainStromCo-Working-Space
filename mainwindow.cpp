 ...
 */
//qDeleteAll(ui->CardsLayout->findChildren<Card*>("", Qt::FindDirectChildrenOnly));
qDeleteAll(ui->scrollAreaWidgetContents->findChildren<Card *>(QString(), Qt::FindDirectChildrenOnly));
/*
std::list<Card*> dataSet;

for(int i =0 ; i<5 ; i++){
    Card *card = new Card();
    card->setOfficePrice("25");
    dataSet.insert(dataSet.begin(), card);
...
for(int i =0 ; i<5 ; i++){
   ...
   QString price = "25000";
   Office * office = new Office(image,description,price);
