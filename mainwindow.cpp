...
QStackedWidget *stack;
  ...
  for(int i=0;i<3;i++){
    for( j=0;j<3;j++){
      stack = new QStackedWidget();
        ...
        QLabel *description = new  QLabel("text description");
        QLabel *img = new  QLabel();
         img->setStyleSheet("maxwidth:50px;height:50px;background-color:white;");
         button5->setText(t[i]);
         button5->setStyleSheet("background-color:red;width:30px;height:10px;");
 ...
 ui->list_of_offices->setLayout(mainLayout);
 */

   QVBoxLayout *column = new QVBoxLayout;
   QHBoxLayout *row = new QHBoxLayout;
   QStackedWidget *item = new QStackedWidget;
   QStackedWidget *rowHolder = new QStackedWidget;
   QStackedWidget *imageHolder = new QStackedWidget;
   QStackedWidget *descriptionHolder = new QStackedWidget;
   QStackedWidget *buttonHolder = new QStackedWidget;
   QLabel *image = new QLabel;
   image->setStyleSheet("maxwidth:50px;height:50px;background-color:white;");
   QLabel *description = new QLabel("description");
   QPushButton *button = new QPushButton("see");
   button->setStyleSheet("background-color:red;width:30px;height:10px;");
   int i = 0 , j= 0 ;
   imageHolder->addWidget(image);
   descriptionHolder->addWidget(description);
   buttonHolder->addWidget(button);
   item->addWidget(imageHolder);
   item->addWidget(descriptionHolder);
   item->addWidget(buttonHolder);
   row->addWidget(item);
   rowHolder->setLayout(row);
   column->addWidget(rowHolder);
   ui->list_of_offices->setLayout(column);
}
