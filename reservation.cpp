...
std::list<Reservation*>Reservation:: getAllOfficesReservation(){
  ...
  QSqlQuery query;
      ...
      while (query.next()) {
        //  QMessageBox::information(nullptr,"test",query.value(3).toString());
...
//select extract(month from check_in)from reservation where id_office='107';;
int Reservation::getMonthcheck_in(QString id_office)
{

    QSqlQuery query;
  query.exec("select extract(month from check_in)from reservation where id_office='"+id_office+"'");
  int check_in =query.value(0).toInt();
  return check_in;
}
int Reservation::getMonthcheck_out(QString id_office)
{

QSqlQuery query;
query.exec("select extract(month from check_out)from reservation where id_office='"+id_office+"'");
int check_out =query.value(0).toInt();
return check_out;
